  ...
  fetch("https://pokeapi.co/api/v2/pokemon/?limit=1126")
    ...
    .then((pokemonNames) => {
      for (i = 0; i < pokemonNames.results.length; i += 1) {
        if (pokemonSearchInput.value.toLowerCase() == pokemonNames.results[i].name) {
          ...
          break;
        } else if (i == pokemonNames.results.length - 1 && pokemonSearchInput.value.toLowerCase() != pokemonNames.results[i - 1].name){
